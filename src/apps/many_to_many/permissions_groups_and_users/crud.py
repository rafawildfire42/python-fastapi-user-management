...
def create_user_permissions_group_relation(db: Session, data: UserAndGroupRelation):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"Error while creating relation between permissions groups and user. Check if Permissions Group #{permissions_group_id} and User #{user_id} exists.")

    except IntegrityError as e:
        logging.error(e)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"Relation between Permissions Group #{permissions_group_id} and User #{user_id} already exists.")

    except Exception as e:
        logging.error(e)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"Error while creating relation between Permissions Group #{permissions_group_id} and User #{user_id}.")
